...
function AdminEditProduct({ id, setActiveComponent ,showAlert}) {
  ...
  const handleUpdate = async () => {
    ...
    formData.append("product", JSON.stringify(updatedProduct)); // Append product data as JSON
      ...
      <form id="editForm">
        ...
        <FormControl fullWidth margin="normal">
          ...
          >
            ...
            <MenuItem value="new">Add New Manufacturer</MenuItem>
          </Select>
          {newManufacturer && (
            <TextField
              fullWidth
        ...
        <Grid container spacing={2}>
          {product.product_image &&
            product.product_image.map((image, index) => (
              <Grid item xs={4} key={index}>
                <img
                  src={`data:image/${image.type};base64,${image.imageData}`}
          ...
          {product.mainImage && (
            <Grid item xs={12} mt={2}>
              <Box>
                <img
                  src={`data:image/${product.mainImage.type};base64,${product.mainImage.imageData}`}
