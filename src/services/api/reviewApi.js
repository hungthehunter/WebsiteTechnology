...
const REVIEW_ENDPOINTS = {
  ...
  GET_ALL_REVIEW_BY_PRODUCT_ID: "/reviews/product/:id",
...
const reviewApi = {
  ...
  getAllReviewByProductId: async (id, LoadingScreen = true) => {
    const res = await shopAPI.get(REVIEW_ENDPOINTS.GET_ALL_REVIEW_BY_PRODUCT_ID.replace(":id", id));
    if (LoadingScreen) {
      await new Promise((state) => setTimeout(state, 2000));
    }
    return res;
  ...
  getReviewById: async (id, LoadingScreen = true) => {
    const res = await shopAPI.get(
      REVIEW_ENDPOINTS.GET_REVIEW_BY_ID.replace(":id", id)
    );
    if (LoadingScreen) {
      await new Promise((state) => setTimeout(state, 2000));
    }
  ...
  updateReview: async (id, reviewData, LoadingScreen = true) => {
    const res = await shopAPI.put(
      REVIEW_ENDPOINTS.UPDATE_REVIEW.replace(":id", id),
      reviewData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
  ...
  deleteReview: async (id, LoadingScreen) => {
    const res = await shopAPI.delete(
      REVIEW_ENDPOINTS.DELETE_REVIEW.replace(":id", id)
    );
    if (LoadingScreen) {
      await new Promise((state) => setTimeout(state, 2000));
    }
